		...
		<tr>
			<td colspan='8' bgcolor=#337ab7 style='border: none; color: white;'>
				...
				<a href='exportExcel.php'>
					<button style='margin-left:230px;' class='btn'><span class='glyphicon glyphicon-print'></span> Cetak</button>
				</a>
			</td>
		</tr>
		<tr>
...
while ($data = mysql_fetch_array($hasil)) {
	...
	// Join untuk mengambil groupName berdasarkan groupId
	$groupQuery = mysql_fetch_array(mysql_query("SELECT groupName FROM tgroup WHERE groupId = '{$data['groupId']}'"));
	$groupName = $groupQuery['groupName']; // Ambil Nama Grup
...
$data_count = mysql_fetch_array(mysql_query("SELECT SUM(jawabanA) As TotalA,
						...
						SUM(jawabanC) As TotalC,
						SUM(jawabanD) As TotalD,
						SUM(jawabanE) As TotalE FROM tanswer"));
